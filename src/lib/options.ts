...
type MappedType<T> = T extends { type: 'slider' } ? number : T extends { type: 'text' } ? string : never;

type MappedName<T> = T extends { name: infer N } ? N extends string ? N : never : never;
